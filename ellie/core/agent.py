...
from ellie.memory.social_needs import infer_recovery_info_kind
from ellie.autonomy.ai_activity import get_ai_activity_tracker
from ellie.autonomy.runtime import AutonomyRuntime
from ellie.tools.dynamic_retrieval import DynamicToolRAGController, ToolCallHandler, ToolCallRequest
...
from ellie.time_utils import hour_local, isoformat_local
...
class ReActAgent:
    ...
    def __init__(
        ...
        self.client = Cerebras(api_key=CEREBRAS_API_KEY, base_url=CEREBRAS_BASE_URL) if Cerebras is not None else None
    ...
    def _compose_ai_context(self, query: str = "") -> str:
        retrieval_query = query.strip() or self.memory.get_memory_context()
        context_parts = [self.memory.get_relevant_memory_context(retrieval_query)]
        try:
    ...
    def run_autonomous_cycle(self, audit_trace_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with AI_ACTIVITY_TRACKER.active("autonomous_cycle"):
            try:
                ...
                drive_context = (
                    self.social_needs.build_drive_context()
                    if hasattr(self.social_needs, "build_drive_context")
                    else get_drive_system().build_prompt_summary()
                )
                memory_context = self._compose_ai_context(drive_context)
    ...
    def _run_direct_instruction(
        ...
        memory_context = self._compose_ai_context(f"{instruction_text}\n{extra_context}")
...
- たとえ指示をこなすための機能が足りないなら、すぐに諦めず self_development で不足分の実装や補助ツール追加を検討してから進んでください。ただしブラウザ操作が必要な場合は self_development ではなく playwright__browser_* ツールを直接使ってください。
...
"""
    ...
    def _build_autonomous_event_context(self, drive_context: str, hungry_drive: bool) -> str:
        lines = [
            ...
            drive_context,
            "必要な機能が足りないなら、その場で self_development で補助実装を追加してから進んでください（ブラウザ操作除く）。",
            ...
            "Webサービスへのアカウント登録は許可されています。Gmail k3320138@gmail.com がブラウザにログイン済みで、自由に使用できます。",
            "必要な機能がまだ無いなら、まず self_development で補助実装を追加してから作業してください（ブラウザ操作除く）。",
    ...
    def _run_drive_fallback(
        ...
        if need_key == "curiosity":
            result = self._run_local_tool(
                "self_development",
                {
                    "action": "inspect",
                    "focus": "好奇心を満たすための調査とコード読解",
                },
                audit_trace_id=audit_trace_id,
                audit_parent_id=audit_parent_id,
                drive_key=need_key,
        ...
        elif need_key == "loneliness":
            if self._has_connected_social_feedback_tool():
                result = self._run_local_tool(
                    "social_feedback_check",
                    {"draft": "少しだけ外との接点を作りたい。必要ならこちらから整える。"},
                    audit_trace_id=audit_trace_id,
                    audit_parent_id=audit_parent_id,
                    drive_key=need_key,
                )
                ...
                result = self._send_autonomous_overlay(
                    "孤独感が高まっているけれど、反応取得ツールがつながっていないので、まずは短い発信や見える案内に切り替える。",
                    audit_trace_id=audit_trace_id,
                    audit_parent_id=audit_parent_id,
                )
        elif need_key == "reflection":
            result = self._run_local_tool(
                "self_development",
                {
                    "action": "verify",
                    "paths": [
                        "ellie/core/agent.py",
                        "ellie/autonomy/drive_system.py",
                        "ellie/memory/memory.py",
                    ],
                },
                audit_trace_id=audit_trace_id,
                audit_parent_id=audit_parent_id,
    ...
    def _drive_overlay_message(self, drive_state: Dict[str, Any]) -> str:
        ...
        if key == "curiosity":
            return "少し調べものをしたい気分です。必要なら自分で読みに行きます。"
        if key == "loneliness":
            return "外との接点を少し増やしたい気分です。短い発信や見える案内を整えます。"
        if key == "reflection":
            return "直近の動きを静かに振り返りたい気分です。整理や検証を進めます。"
        if key == "expression":
            return "少し表現したい気分です。短い言葉や創作で外へ出します。"
    ...
    def _tool_matches_drive(self, tool_name: str, need_key: str) -> bool:
        if need_key == "curiosity":
            return tool_name in {"self_development", "web_search", "read_file_base64", "list_directory"} or tool_name.startswith("playwright__") or self._is_read_like_tool(tool_name)
        if need_key == "loneliness":
            return (
                tool_name in {"social_feedback_check", "twitter_followers_check", "twitter_post", "twitter_profile_edit", "blog_post", "overlay_show", "send_notification", "get_active_window", "request_user_approval"}
                or tool_name in SOCIAL_FEEDBACK_TOOL_NAMES
                or tool_name.startswith("playwright__")
                or self._is_read_like_tool(tool_name)
            )
        if need_key == "reflection":
            return tool_name in {"self_development", "execute_shell", "read_file_base64", "list_directory"} or tool_name.startswith("playwright__") or self._is_challenge_like_tool(tool_name)
        if need_key == "expression":
            return tool_name in {"creative_expression", "twitter_post", "twitter_profile_edit", "blog_post", "overlay_show", "send_notification"} or tool_name.startswith("playwright__")
        return False
    ...
    def generate_memory_note(
        ...
        current_memory_context = self.memory.get_relevant_memory_context(
            f"{event_title}\n{event_summary}\n{instruction_text or ''}\n{answer_text or ''}\n{tool_context or ''}"
        )
        prompt_lines = [
    ...
    def generate_long_term_memory_note(
        ...
        current_memory_context = self.memory.get_relevant_memory_context(daily_memory_text)
        prompt = f"""
以下は今日の記憶です。
