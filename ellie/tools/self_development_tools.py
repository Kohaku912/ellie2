...
def _self_development_inspect(arguments: JsonDict) -> JsonDict:
    ...
    suggestions.append("利用可能なツールカテゴリ:")
    suggestions.append("  🔍 Search Tools: web_search, agent_grep_search, agent_file_search, agent_read_file, list_directory")
    suggestions.append("  🧠 Memory Tools: agent_add_memory, agent_search_memory, schedule_self_call, create/update_long_term_goal")
    suggestions.append("  📁 File Tools: agent_replace_string, agent_insert_text, agent_create_file, read_file_base64, execute_shell")
    suggestions.append("  🌐 Browser: playwright__browser_navigate/click/type/snapshot 等（self_development禁止）")
    suggestions.append("  🔧 Self-Development: self_development(inspect/write_file/verify/request), self_restart, overlay_show")
    suggestions.append("")
    suggestions.append("新規Tool追加の手順:")
    suggestions.append("  1. 既存の類似Toolを agent_read_file で読んでパターンを把握する")
    suggestions.append("  2. 該当ファイルを agent_insert_text または agent_replace_string で編集する")
    suggestions.append("  3. registry.py に ToolDefinition を追加する（必要な場合）")
    suggestions.append("  4. dynamic_retrieval.py に handler を追加する（必要な場合）")
    suggestions.append("  5. execute_shell で py_compile を実行して検証する")
    suggestions.append("  6. execute_shell で pytest を実行して回帰テストを行う")
    suggestions.append("")
    suggestions.append("【絶対禁止】ブラウザ操作のための新ツールを self_development で作成しないでください。")
    suggestions.append("ブラウザ操作は playwright__browser_navigate / _click / _type / _snapshot 等の")
    suggestions.append("既存の Playwright MCP ツールを直接組み合わせて実現してください。")
