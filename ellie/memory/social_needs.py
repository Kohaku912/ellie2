...
from ellie.time_utils import agent_tz, now_local
...
class SocialNeedsManager:
    ...
    def apply_user_message(self, text: str) -> None:
        ...
        self.decay_to_now()
        message = text or ""
        get_drive_system().note_user_message(message)
    ...
    def apply_activity_event(
        ...
        get_drive_system().note_activity(
            normalized_event,
    ...
    def build_drive_context(self) -> str:
        """Build action-oriented context that maps drive pressure to satisfying tools."""
        self.decay_to_now()
        drive_summary = get_drive_system().build_prompt_summary()
        social_drive_states = [self._drive_state_for(key, need) for key, need in self.needs.items()]
        hungry_states = [state for state in social_drive_states if state["should_act"]]
        hungry_states.sort(key=lambda state: state["drive_intensity"], reverse=True)

        lines = [
            drive_summary,
            "## 社会的欲求の補足",
            "- 共感・承認・探求・挑戦の回復は従来どおりここで扱う。",
            "- 孤独感に近い圧力は DriveSystem の loneliness に集約され、ここでは重複して扱わない。",
        ]
        ...
        if hungry_states:
            ...
            for state in hungry_states:
                lines.extend(
                    [
                        f"- {state['label']}: {state['recommended_tools'][0] if state['recommended_tools'] else 'tool'} を含む候補が有効です。",
    ...
    def get_debug_snapshot(self) -> Dict[str, Any]:
        ...
        snapshot["_drive_system"] = get_drive_system().get_debug_snapshot()
        return snapshot

    def get_drive_states(self) -> list[Dict[str, Any]]:
        """Return drive readiness from the dedicated drive system."""
        return get_drive_system().get_drive_states()

    def mark_drive_action(self, need_key: str) -> None:
        """Record that an autonomous action was attempted for a drive."""
        get_drive_system().mark_action(need_key)

    def mark_drive_actions(self, need_keys: list[str]) -> None:
        """Record several autonomous drive actions with one save."""
        get_drive_system().mark_actions(need_keys)

    def _tone_lines(self) -> list[str]:
        lines: list[str] = []
        if self.empathy.delta < -DEVIATION_THRESHOLD:
            lines.append(
                "- 共感の伸びしろ：最近の対話では、相手の感情をもう少し丁寧に拾うと応答が自然になります。"
            )
